...
export default class Album extends React.Component {
  ...
  isReady (customAlbum = null, customArtists = null) {
    const album = customAlbum === null ? this.props.album : customAlbum;
    const artists = customArtists === null ? this.props.artists : customArtists;
  ...
  tracks () {
    ...
    return (
      <div>
        <span className={styles['track-count']}>
          {`${album.tracks.total} Track(s)`}
        </span>
            ...
            return (
              <li key={track.id}>
                <Track
                  track={track}
                  isSaved={isSaved}
                  onToggleSave={() => this.toggleSaveTrack(track)}
                />
              </li>
            );
          })}
...
Album.propTypes = {
  ...
  savedTracks: PropTypes.shape({}).isRequired,
